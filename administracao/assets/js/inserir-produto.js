...
function elimarImagem() {
	...
	$.ajax({
        ...
        data: {'produto': produto, 'imagem': imagem},
...
function associarProduto(produto_associado) {
	...
	$.ajax({
		...
		data: {'produto': produto, 'produto_associado': produto_associado},
		success: function(response) {
			$('#modal-associar-produto').modal('hide');
			$('#grid2').load('php/int/int-associados.php?produto=' + produto).fadeIn('slow');
		}
	});
}
