    ...
    <body>
        ...
        <main class="main-wrap">
            ...
            <section class="content-main">
                <div class="content-header">
                    <div>
                        <h2 class="content-title card-title">Clientes</h2>
                        <p>Todos os clientes registados na sua loja.</p>
                    </div>
                </div>
                    ...
                    <header class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-3 col-6">
                                <select class="form-select" name="distrito" id="distrito" onchange="filtrarRegistos(10, 1)">
                                    <option value="0" selected>Distrito</option>
                                    <?php
                                        $result_distristo = $pdo->prepare("SELECT * FROM distrito ORDER BY nome ASC");
                                        $result_distristo->execute();
                                        $row_distrito = $result_distristo->fetch();
                                        if ($result_distristo->rowCount()>0) {
                                            for ($i=1; $i<=$result_distristo->rowCount(); $i++) {
                                                echo '<option value="'.$row_distrito['nome'].'">'.$row_distrito['nome'].'</option>';
                                                $row_distrito = $result_distristo->fetch();
                                            }
                                        }
                                    ?>
        ...
        <script>
            ...
            function mostrarRegistos(registos, pagina) {
                $.ajax({
                    type: 'POST',
                    url: 'php/int/int-clientes.php?mostrar=' + registos + '&pagina=' + pagina,
                    data: {},
                    cache: false,
                    success: function(response) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'php/int/paginacao-clientes.php?mostrar=' + registos + '&pagina=' + pagina,
                            data: {},
                            cache: false,
                            success: function(response) {
            ...
            function filtrarRegistos(registos, pagina) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'php/int/int-clientes.php?mostrar=' + registos + '&pagina=' + pagina,
                    data: {'distrito': distrito, 'filtro': filtro},
                    cache: false,
                    success: function(response) {
                        $('#int-registos').html(response);
                        $.ajax({
