<?php
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        $result = $pdo->prepare("SELECT * FROM voucher WHERE id_voucher = :voucher");
        $result->execute(array('voucher' => $_GET['id']));
        $row = $result->fetch();
...
<html lang="pt-pt">
    ...
    <body>
        ...
        <main class="main-wrap">
            ...
            <form id="form-editar-voucher">
                <section class="content-main">
                    <div class="row">
                        <div class="col-12">
                            <div class="content-header">
                                <h2 class="content-title">Editar Voucher - <?= $row['codigo'];?></h2>
                                <div>
                            ...
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-lg-4">
                                            <div class="mb-4">
                                                <input type="hidden" name="voucher" value="<?= $row['id_voucher'];?>">
                                                <label for="codigo" class="form-label">Código *</label>
                                                <div class="row gx-2" style="position: relative;">
                                                    <input placeholder="Insira o código" type="text" class="form-control" name="codigo" id="codigo" onblur="verificarCodigo(<?= $row['id_voucher'];?>)" value="<?= $row['codigo'];?>">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <div class="row gx-2">
                                                    <select name="tipo_desconto" id="tipo" class="form-control form-select">
                                                        ...
                                                        ?>
                                                        <option value="1"<?= $tipo1;?>>Valor (€)</option>
                                                        <option value="2"<?= $tipo2;?>>Percentagem (%)</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-lg-4">
                                            <div class="mb-4">
                                                ...
                                                <div class="row gx-2">
                                                    <input type="number" name="desconto" id="valor" class="form-control" value="<?= $row['desconto'];?>">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-lg-3">
                                            <div class="mb-4">
                                                ...
                                                <div class="row gx-2">
                                                    <input type="number" name="valor_minimo" id="valor-minimo" class="form-control" value="<?= $row['valor_minimo'];?>">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-lg-3">
                                            <div class="mb-4">
                                                <label for="limite-utilizacoes" class="form-label">Número Limite de Utilizações | Já utilizado <?= $row['utilizacoes'];?> vezes</label>
                                                <div class="row gx-2">
                                                    <input type="number" name="limite_utilizacoes" id="limite_utilizacoes" class="form-control" value="<?= $row['limite_utilizacoes'];?>">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-lg-3">
                                            <div class="mb-4">
                                                ...
                                                <div class="row gx-2">
                                                    <?php
                                                        if (!is_null($row['data_expiracao'])) {
                                                            $data_expiracao = $row['data_expiracao'];
                                                        } else {
                                                            $data_expiracao = "";
                                                        }
                                                    ?>
                                                    <input type="date" name="data_expiracao" id="data-expiracao" class="form-control" value="<?= $data_expiracao;?>">
                                                </div>
                                            </div>
                                        </div>
                                                    ...
                                                    <select name="estado" id="estado" class="form-control form-select">
                                                        <?php
                                                            ...
                                                            if ($row['estado']==1) {
                                                                ...
                                                                $estado3 = '<option value="2">Expirado</option>';
                                                            }
                                                        ?>
                                                        <option value="1"<?= $estado1;?>>Ativo</option>
                                                        <option value="0"<?= $estado2;?>>Inativo</option>
                                                        <?= $estado3;?>
                                                    </select>
                                                </div>
                                            </div>
                                                        ...
                                                        <?php
                                                            ...
                                                            if ($result_cliente->rowCount()>0) {
                                                                for ($i=1; $i<=$result_cliente->rowCount(); $i++) {
                                                                    if ($row_cliente['id_cliente']==$row['cliente']) {
                                                                        $selected = " selected";
                                                                    } else {
                                                                        $selected = "";
                                                                    }
                                                                    echo '<option value="'.$row_cliente['id_cliente'].'"'.$selected.'>'.$row_cliente['nome'].'</option>';
                                                                }
                                                            }
                                                        ?>
                                                    </select>
        ...
        <script>
            function verificarCodigo(voucher) {
                var codigo = $('#codigo').val();
                var voucher = voucher;
                $.ajax({
                    type: 'POST',
                    url: 'php/verificar/verificar-voucher.php',
                    data: {'codigo': codigo, 'voucher': voucher},
                    cache: false,
            ...
            $(document).ready(function() {
                var optionsEditar = {
                    ...
                    url: 'php/editar/processa-editar-voucher.php',
                    type: 'post',
                    clearForm: 0,
                    resetForm: 0
                };
                $('#form-editar-voucher').ajaxForm(optionsEditar);

                function showResponseEditar(msg) {
                    if (msg=='') {
                        window.location.href = 'vouchers.php?e=1';
                    } else {
                        $(this).html(msg);
                        $('html, body').animate({
                            scrollTop: 0
