<?php
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        $result = $pdo->prepare("SELECT * FROM mensagem WHERE id_mensagem = :mensagem");
        $result->execute(array('mensagem' => $_GET['id']));
        $row = $result->fetch();
        if ($result->rowCount()==0) {
            header("Location: mensagens.php");
        }
        $query = $pdo->prepare("UPDATE notificacao SET estado = 1 WHERE destino = :destino AND tipo = 5");
        $query->execute(array('destino' => $_GET['id']));
    } else {
        header("Location: mensagens.php");
    }
?>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="content-header">
                            <h2 class="content-title">Mensagem - <?= $row['nome'];?></h2>
                            <div>
                                <?php
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-4">
                                            ...
                                            <input type="text" class="form-control" value="<?= $row['assunto'];?>" disabled>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="mb-4">
                                            ...
                                            <textarea class="form-control" disabled><?= str_replace('<br />', "", $row['texto']);?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
