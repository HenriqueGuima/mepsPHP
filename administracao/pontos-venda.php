    ...
    <main class="main-wrap">
        ...
        <section class="content-main">
            <div class="content-header">
                <div>
                    <h2 class="content-title card-title">Pontos de Venda</h2>
                    <p>Todos os pontos de venda.</p>
                </div>
            </div>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="align-middle" scope="col">Localidade</th>
                                    ...
                                    <th class="align-middle text-end" scope="col">Ver</th>
                                </tr>
                            </thead>

                            <tbody>
                                <?php
                                        $result_ponto_venda = $pdo->prepare("SELECT * FROM ponto_venda ORDER BY id_ponto_venda DESC");
                                        $result_ponto_venda->execute();
                                        $row_ponto_venda = $result_ponto_venda->fetch();
                                        if ($result_ponto_venda->rowCount()>0) {
                                            for ($i=1; $i<=$result_ponto_venda->rowCount(); $i++) {
                                                echo '<tr>
                                                        <td>'.$row_ponto_venda['nome'].'</td>
                                                        <td>'.$row_ponto_venda['morada'].'</td>
                                                        <td>'.$row_ponto_venda['localidade'].'</td>
                                                        <td>'.$row_ponto_venda['telefone'].'</td>
                                                        <td>'.$row_ponto_venda['email'].'</td>
                                                        <td>'.$row_ponto_venda['estado'].'</td>
                                                        <td class="text-end"><a href="ponto_venda.php?id='.$row_ponto_venda['id_ponto_venda'].'" class="btn btn-xs"> Detalhes</a></td>
                                                      </tr>';
                                                $row_ponto_venda = $result_ponto_venda->fetch();
                                            }
                                        }
